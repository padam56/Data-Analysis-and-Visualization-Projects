...
# TYPES OF PLOTS DISCUSSED HERE: SCATTER PLOT, LINE PLOT, COLUMN PLOT & HISTOGRAM.
...
store %>% group_by(PRODUCT_TYPE) %>% 
  ...
  ggplot(aes(x=PRODUCT_TYPE, y=AVERAGE_NET_PROFIT))+geom_col(fill="light blue")+theme(element_text(size=9))
...
store %>% group_by(PRODUCT_CATEGORY) %>% 
  ...
  ggplot(aes(x=PRODUCT_CATEGORY, y=AVERAGE_QUANTITY, group=1))+geom_line(color="GREEN")+theme(text = element_text(size = 4.5))
